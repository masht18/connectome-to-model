...
model_dir="saved_models/4task"
hstate_dir="saved_models/hstates_avs"
graph_dir="graphs/4task_models"
...
for graph in "${graphs[@]}"; do
  for seed in "${seeds[@]}"; do
    ...
    python scripts/multimodal_all_scenarios.py \
      --seed "$seed" \
      --epochs 1 \
      --graph_loc "${graph_dir}/multimodal_${graph}.csv" \
